...
/**
 * Email envoyé au client dès la création d'une commande payée par carte bancaire.
 * La commande existe côté serveur mais le paiement n'est pas encore validé.
 */
export function cardOrderPlacedEmail({ customerName, order, trackingUrl }) {
    const itemsRows = (order.items || [])
        .map(
            (it) => `
        <div class="info-row">
          <span class="label">${it.productSnapshot?.name || "Produit"} × ${it.quantity}</span>
          <span class="value">${formatPrice(it.totalPrice)}</span>
        </div>`
        )
        .join("")

    return {
        subject: `Commande reçue — #${order.orderNumber} (paiement par carte en cours)`,
        html: emailLayout({
            title: "Commande reçue",
            preheader: `Votre commande #${order.orderNumber} a bien été enregistrée.`,
            content: `
        <h2>Merci pour votre commande !</h2>
        <p>Bonjour ${customerName},</p>
        <p>
          Nous avons bien reçu votre commande <strong>#${order.orderNumber}</strong>.
          Votre paiement par <strong>carte bancaire</strong> est en cours de vérification
          par notre équipe. Vous recevrez un email dès qu'il sera validé.
        </p>
        ...
        <div class="info-box">
          ...
          <div class="info-row">
            ...
            <span class="value">${order.shippingCost === 0 ? "Gratuite" : formatPrice(order.shippingCost)}</span>
          </div>
          <div class="total-row">
            <span>Total</span>
        ...
        <p>Vous pouvez suivre l'avancement de votre commande à tout moment :</p>
        ...
        <p style="color:#71717a;font-size:13px;">
          Si vous n'êtes pas à l'origine de cette commande, contactez notre support.
        </p>
      `,
        }),
    }
