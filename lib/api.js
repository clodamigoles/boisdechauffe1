...
/**
 * La langue de la page en cours, telle que le navigateur la voit.
 *
 * Les routes `/api` ne passent pas par le routage multilingue de Next : elles
 * ne savent pas dans quelle langue la page qui les appelle est affichée. Sans
 * cette information, elles retombent sur le cookie `NEXT_LOCALE` — qui n'existe
 * que si le visiteur a cliqué sur le sélecteur de langue — puis sur l'allemand.
 *
 * Résultat avant ce correctif : un visiteur qui arrivait directement sur
 * `/fr/shop` par un lien voyait une page française remplie de descriptions
 * allemandes.
 *
 * `document.documentElement.lang` est posé par `_document` à partir de la
 * langue résolue par Next : c'est la source la plus fiable, et elle ne demande
 * pas de faire descendre le routeur jusqu'ici.
 */
function currentLocale() {
    if (typeof document === 'undefined') return null
    const lang = document.documentElement.lang
    return lang === 'fr' || lang === 'de' ? lang : null
}

/** Ajoute `locale` à une URL d'API qui ne le porte pas déjà. */
export function withLocale(endpoint, locale = currentLocale()) {
    if (!locale || endpoint.includes('locale=')) return endpoint
    return `${endpoint}${endpoint.includes('?') ? '&' : '?'}locale=${locale}`
}

export const apiCall = async (endpoint, options = {}) => {
    const path = withLocale(endpoint)
...
export /** La langue à laquelle une entrée de cache appartient. */
...
const cacheUtils = {
    ...
    /**
     * Les clés portent la langue.
     *
     * Sans elle, le catalogue mis en cache en allemand était resservi tel quel
     * après un passage en français : la page changeait de langue, les noms de
     * produits non. Le cache vit trente à soixante minutes, la bascule aurait
     * donc duré.
     */
    keys: {
        categories: (params = {}) => `categories_${cacheLocale()}_${JSON.stringify(params)}`,
        products: (filters = {}) => `products_${cacheLocale()}_${JSON.stringify(filters)}`,
        testimonials: (params = {}) => `testimonials_${cacheLocale()}_${JSON.stringify(params)}`
    },
...
export const cachedAPI = {
    categories: {
        ...
        getFeatured: async (useCache = true) => {
            const cacheKey = `categories_featured_${cacheLocale()}`
    ...
    products: {
        ...
        getFeatured: async (filter = 'all', limit = 8, useCache = true) => {
            const cacheKey = `products_featured_${cacheLocale()}_${filter}_${limit}`
    ...
    testimonials: {
        getFeatured: async (limit = 6, useCache = true) => {
            const cacheKey = `testimonials_featured_${limit}`
            ...
            const result = await testimonialsService.getFeatured(limit)
            if (useCache) cacheUtils.set(cacheKey, result, 30) // Cache 30min
            return result
        },

        getAll: async (params = {}, useCache = true) => {
            const cacheKey = cacheUtils.keys.testimonials(params)
            ...
            const result = await testimonialsService.getAll(params)
            if (useCache) cacheUtils.set(cacheKey, result, 20) // Cache 20min
            return result
        }
    }
}
